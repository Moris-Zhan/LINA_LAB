...
public class YOLOClassifier {
    private final static float OVERLAP_THRESHOLD = 0.4f;
    private final static double anchors[] = {1.08,1.19,  3.42,4.41,  6.63,11.38,  9.42,5.11,  16.62,10.52};
//    private final static double anchors[] = {10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,326};
    private final static int SIZE = 13;
    private final static int MAX_RECOGNIZED_CLASSES = 80;
...
//        model.setWidth(Math.exp(tensorFlowOutput[offset + 2]) * anchors[2 * b]);
//        model.setHeight(Math.exp(tensorFlowOutput[offset + 3]) * anchors[2 * b + 1]);
