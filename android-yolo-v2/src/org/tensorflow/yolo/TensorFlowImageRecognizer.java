...
public class TensorFlowImageRecognizer {
    ...
    public static TensorFlowImageRecognizer create(AssetManager assetManager) {
        ...
        recognizer.labels = ClassAttrProvider.newInstance(assetManager).getLabels(); // read Label
        recognizer.inferenceInterface = new TensorFlowInferenceInterface(assetManager, // read Model
                "file:///android_asset/" + MODEL_FILE);

        final Operation operation = recognizer.inferenceInterface.graphOperation(OUTPUT_NAME);
        final int numClasses = (int) operation.output(0).shape().size(1);
        Log.i(TAG, "Read " + recognizer.labels.size() + " labels, output layer size is " + numClasses);
